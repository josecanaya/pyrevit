...
PARAM_NUMERIC = [
    u"BTZ_Description_{:02d}".format(i) for i in range(1, 81)
]
ALL_BTZ_PARAMS = [PARAM_BASE] + PARAM_NUMERIC

# Metadatos asociación CSV / sugerencia IA (mismo TXT que BTZ_Description)
PARAM_NUMERO_ACTIVO = u"BTZ_NumeroActivo"
PARAM_ESTADO_ASOCIACION = u"BTZ_EstadoAsociacion"
PARAM_ORIGEN_ASOCIACION = u"BTZ_OrigenAsociacion"
PARAM_FECHA_ASOCIACION = u"BTZ_FechaAsociacion"
PARAM_STATUS = u"BTZ_Status"
PARAM_SOURCE = u"BTZ_Source"
PARAM_CONFIDENCE = u"BTZ_Confidence"
ALL_BIND_PARAMS = ALL_BTZ_PARAMS + [
    PARAM_NUMERO_ACTIVO,
...
def ensure_btz_shared_parameters(doc, log_lines):
    """Vincula shared parameters BTZ (BTZ_Description, _01.._80, metadatos, Status/Source/Confidence)."""
...
def get_free_btz_slots(element):
    """BTZ_Description_01..80 vacíos, en orden."""
    empty = []
...
def count_existing_btz_slots(element):
    """Cantidad de slots BTZ_Description_01..80 actualmente ocupados."""
    used = 0
    for pname in PARAM_NUMERIC:
        val = _param_value_as_string(element, pname)
        if (val or u"").strip():
...
def get_existing_btz_values(element):
    """Valores no vacíos en BTZ_Description y _01..80 (para detectar duplicados)."""
    vals = set()
