...
__title__ = "Asignar"
__doc__ = "Asignacion manual jerarquica BTZ (planta/sector/subsector/unidad) + reporte"
...
REPORT_CSV_PATH = os.path.join(PUBLIC_DIR, u"btz_manual_usage_report.csv")
REPORT_TXT_PATH = os.path.join(PUBLIC_DIR, u"btz_manual_usage_report.txt")
...
def _catalog_candidate_paths():
    ...
    base = [
        corrected_alt,
        os.path.join(PUBLIC_DIR, u"sectores_subsectores_btz_manual.csv"),
        os.path.join(RESOURCES_DIR, u"sectores_subsectores_btz_manual.csv"),
    ]
    base.extend(sorted(glob(os.path.join(PUBLIC_DIR, u"sectores_subsectores_btz_manual*.csv"))))
...
def main():
    ...
    if created_sector or created_subsector:
        try:
            append_catalog_entries(
                catalog=catalog,
                output_csv_path=os.path.join(PUBLIC_DIR, u"sectores_subsectores_btz_manual.csv"),
                plant_code=picked[u"plant_code"],
