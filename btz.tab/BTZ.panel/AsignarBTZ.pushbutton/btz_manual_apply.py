...
def apply_manual_hierarchy(
    ...
    for el in elements:
        ...
        na = _u(numero_activo)
        if na:
            cur_na = _u(get_current_value(el, u"BTZ_NumeroActivo")).upper()
            if not cur_na or overwrite:
                okn, errn = set_text_parameter(el, u"BTZ_NumeroActivo", na)
                if not okn:
                    result[u"errors"].append(
                        u"Id {0} BTZ_NumeroActivo: {1}".format(eid, errn or u"error")
                    )
