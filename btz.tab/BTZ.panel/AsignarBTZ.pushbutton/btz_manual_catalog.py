...
def merge_manual_catalogs(target, addition):
    """
    Fusiona `addition` en `target` (muta `target`).
    Misma jerarquía que _build_catalog_from_*: planta -> sector -> subsector -> unidad.
    """
    if not addition.get(u"plants"):
        return
    for pc, p_add in addition[u"plants"].items():
        if pc not in target[u"plants"]:
            target[u"plants"][pc] = copy.deepcopy(p_add)
            continue
        p_t = target[u"plants"][pc]
        if _u(p_add.get(u"name")) and not _u(p_t.get(u"name")):
            p_t[u"name"] = p_add[u"name"]
        for sk, s_add in p_add[u"sectors"].items():
            if sk not in p_t[u"sectors"]:
                p_t[u"sectors"][sk] = copy.deepcopy(s_add)
                continue
            s_t = p_t[u"sectors"][sk]
            if _u(s_add.get(u"name")) and not _u(s_t.get(u"name")):
                s_t[u"name"] = s_add[u"name"]
            if _u(s_add.get(u"write_code")) and not _u(s_t.get(u"write_code")):
                s_t[u"write_code"] = s_add[u"write_code"]
            for subk, sub_add in s_add[u"subsectors"].items():
                if subk not in s_t[u"subsectors"]:
                    s_t[u"subsectors"][subk] = copy.deepcopy(sub_add)
                    continue
                sub_t = s_t[u"subsectors"][subk]
                if _u(sub_add.get(u"name")) and not _u(sub_t.get(u"name")):
                    sub_t[u"name"] = sub_add[u"name"]
                u_add = sub_add.get(u"units") or {}
                if not u_add:
                    continue
                if sub_t.get(u"units") is None:
                    sub_t[u"units"] = {}
                for uk, uo_add in u_add.items():
                    if uk not in sub_t[u"units"]:
                        sub_t[u"units"][uk] = copy.deepcopy(uo_add)
                    else:
                        uo_t = sub_t[u"units"][uk]
                        if _u(uo_add.get(u"name")) and not _u(uo_t.get(u"name")):
                            uo_t[u"name"] = uo_add[u"name"]


def _find_sector_for_general_prefix(plant_obj, plant_code, sector_token):
    st = _u(sector_token).upper()
    sectors = plant_obj.get(u"sectors") or {}
    if st in sectors:
        return sectors[st]
    pc = _u(plant_code).upper()
    needle = u"{0}-{1}".format(pc, st)
    for s in sectors.values():
        wc = _u(s.get(u"write_code")).upper()
        if wc.startswith(needle) or u"-{0}-".format(st) in wc:
            return s
    return None


def merge_general_container_subsectors(catalog):
    """
    Añade subsectores contenedor GENERAL por sector (desde preparar_match_project_revit_p10).
    Solo en memoria al cargar catálogo; no escribe Revit ni aplica hijos.
    """
    _root = os.path.normpath(os.path.join(os.path.dirname(os.path.abspath(__file__)), u"..", u"..", u".."))
    _tools = os.path.join(_root, u"btz_console", u"tools")
    if _tools not in sys.path:
        sys.path.insert(0, _tools)
    try:
        import preparar_match_project_revit_p10 as prep
    except Exception:
        return
    gmap_fn = getattr(prep, u"general_container_map_for_plant", None)
    if not callable(gmap_fn):
        return
    for plant_code, plant_obj in list(catalog.get(u"plants", {}).items()):
        pmap = gmap_fn(plant_code)
        if not pmap:
            continue
        pc = _u(plant_code).upper()
        for prefix, full_general in pmap.items():
            parts = _u(prefix).upper().split(u"-")
            if len(parts) < 2:
                continue
            sec_tok = parts[1]
            s = _find_sector_for_general_prefix(plant_obj, pc, sec_tok)
            if s is None:
                s = _ensure_sector(
                    plant_obj,
                    sec_tok,
                    sec_tok,
                    u"{0}-{1}-LGC-{1}".format(pc, sec_tok),
                )
            if s is None:
                continue
            sub_key = _u(full_general).upper()
            if sub_key in s[u"subsectors"]:
                continue
            tail = u""
            if u"GENERAL" in _u(full_general).upper():
                tail = _u(full_general).upper().split(u"GENERAL", 1)[-1].strip()
            disp = (u"GENERAL " + tail).strip() if tail else _u(full_general)
            s[u"subsectors"][sub_key] = {
                u"code": full_general,
                u"name": disp,
            }
...
def load_manual_catalog(csv_paths):
    ...
    for p in csv_paths:
        ...
        try:
            ...
            continue
        if not catalog.get(u"plants"):
            continue
        if merged is None:
            merged = catalog
            primary_schema = catalog.get(u"schema", schema)
            merged_paths.append(ap)
        else:
            merge_manual_catalogs(merged, catalog)
            merged_paths.append(ap)
    if merged is None or not merged.get(u"plants"):
        raise IOError(u"No se encontró catálogo manual. Rutas buscadas:\n- {0}".format(u"\n- ".join(checked)))
    merged[u"schema"] = primary_schema
    merged[u"merged_source_paths"] = merged_paths
    merged[u"source_path"] = u"merge:{0}".format(u";".join(merged_paths))
    try:
        merge_general_container_subsectors(merged)
    except Exception:
        pass
    return merged
