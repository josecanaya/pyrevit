...
def _resolve_exportar_grupos_dir(bundle_dir):
    """
    ExportarGrupos.pushbutton vive en BTZ.panel.
    - Si este script esta en ...\\BTZ.panel\\AsignarBTZ.pushbutton -> un nivel .. alcanza.
    - Si esta en ...\\Avanzado.stack\\AsignarBTZ.pushbutton -> hace falta subir dos niveles.
    """
    candidates = [
        os.path.normpath(os.path.join(bundle_dir, u"..", u"ExportarGrupos.pushbutton")),
        os.path.normpath(
            os.path.join(bundle_dir, u"..", u"..", u"ExportarGrupos.pushbutton")
        ),
    ]
    for cand in candidates:
        if os.path.isfile(os.path.join(cand, u"btz_paths.py")):
            return cand
    return candidates[0]


_export_dir = _resolve_exportar_grupos_dir(_bundle_dir)
